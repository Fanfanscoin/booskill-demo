...
用户购买、领取、发送、输入、提交、激活或使用 BossSkill 授权码，即表示已阅读、理解并同意：

- [BossSkill 用户协议](USER_AGREEMENT.md)
- [BossSkill 隐私与数据说明](PRIVACY.md)
- [授权码使用说明](LICENSE_NOTICE.md)

## 隐私与数据传输

BossSkill 默认使用本地 SQLite 保存客户、团队、任务、人脉等资料。授权校验只向授权中心发送授权码、设备标识和要校验的功能名，不要求客户上传本地数据库。

调用授权版云端能力时，客户端默认只发送当次命令参数，例如用户本次输入的问题、命令名和授权信息；不会自动上传完整客户库、团队库、任务库或本地 SQLite 文件。只有当用户明确要求生成公网下载链接时，才会上传用户指定的 Word/PDF 文件到 `docs.fanfan.la`，且链接默认短期有效。
...
- 商业版：诊断、决策、排序、策略、追结果、执行力训练、行业包、长期学习和经营闭环。
